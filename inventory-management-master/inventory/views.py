...
#def about_to_expire(request):
    ...
    #expiring_products = Stock.objects.filter(expiry_date__lte=threshold_date, is_deleted=False).order_by('expiry_date')
    #return render(request, 'inventory/about_to_expire.html', {'expiring_products': expiring_products})

def about_to_expire(request):
    today = timezone.now().date()
    cutoff_date = today + timedelta(days=30)
    products = Stock.objects.filter(expiry_date__lte=cutoff_date)
    context = {
        'products': products,
    }
    return render(request, 'inventory/about_to_expire.html', context)
...
def download_csv(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="products_close_to_expiration.csv"'

    writer = csv.writer(response)
    writer.writerow(['Stock Name', 'Current Stock', 'Expiry Date'])

    for product in products:
        writer.writerow([product.name, product.quantity, product.expiry_date])
